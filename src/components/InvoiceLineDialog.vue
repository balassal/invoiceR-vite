      ...
      <q-card-section class="row" style="max-height: 60vh;overflow-y: auto;">
        <div class="col-8">
          <div class="row items-center">
            ...
            <div class="col-9">
              <q-select dense clearable :options="products" v-model="state.productId" @update:model-value="onProductChange" />
            </div>
          </div>
          <div class="row items-center">
            <div class="col-3">
          ...
          <div class="row items-center">
            ...
            <div class="col-9">
              <div class="row items-center">
                <div class="col">
                  <q-input dense clearable type="number" v-model.number="state.quantity" @update:model-value="calculate" />
                </div>
                <div class="col">
                  <q-input class="q-pl-md" dense clearable label="Unit of Measure" v-model="state.uomId" />
                </div>
              </div>
            </div>
          </div>
          <div class="row items-center">
            ...
            <div class="col-9">
              <q-input type="number" dense clearable v-model="state.unitPrice" @update:model-value="calculate" />
            </div>
          </div>
          <div class="row items-center">
            <div class="col-3">
            ...
            <div class="col-9">
              <q-select dense clearable use-chips multiple :options="taxes" v-model="state.taxesIds" @update:model-value="calculate" />
            </div>
          </div>
        </div>
        <div class="col">
          <table class="full-width">
            <tbody>
              <tr>
                ...
                <td class="text-right">{{ state.netAmount.toLocaleString("hu-Hu") }}</td>
              </tr>
              <tr v-for="tax in state.taxesIds" :key="tax.id">
                <th class="text-right">{{ tax.label }}</th>
                <td class="text-right">{{ (state.netAmount * tax.amount).toLocaleString("hu-HU") }}</td>
              </tr>
              <tr>
                <th class="text-right">Total</th>
                <td class="text-right">{{ state.total.toLocaleString("hu-HU") }}</td>
              </tr>
            </tbody>
          </table>
        </div>
